...
**Status:** 2026-08-21, after R12 (regulator restored, single admin).
**Roles:** five — `admin`, `regulator`, `analyst`, `field_officer` (**Data Submitter**),
`citizen` (**Resident**).
...
> **`regulator` was retired in R7 and RESTORED in R12** (migrations `0019`, `0022`).
> R7's reasoning was that `admin` already held all four of its powers, so it was a
> second label on one authority. R12 reverses that, because the reasoning had it
> backwards: the powers should never have been the same. Accepting a field officer's
> evidence into the record and operating the pipeline that consumes it are different
> jobs, and collapsing them meant the only person who could approve a submission was
> also the only person who could rewrite the file it lands in.
>
> The restored role is **narrower than the original**: review the queue, approve,
> reject. It cannot sync or seed `Datasets/`, factory reset, run model operations,
> read the audit log, publish an advisory, or create an admin.
>
> **There may be many regulators. There is exactly one admin** — enforced in
> `UserService._refuse_second_admin` and again by the `uq_single_admin` partial index
> in migration `0022`, because an application check is a race between two requests and
> an index is not. The admin is created by `python -m scripts.bootstrap_admin`, never
> by the seed.
>
> The UI labels are not the enum names: `field_officer` is shown as **Data
...
publishes or archives an advisory, reads the audit log, and operates the dataset pipeline. An
analyst may generate a frightening number; only an admin can make it public. This is why
`GET /audit` is admin-only — analysts appear in the log and cannot read it, and so does a
regulator, whose own decisions are written there.

Since R12 the guards are spelled for what they protect: `require_field_reviewer`
(admin + regulator) decides on a submission, `require_reviewer` (admin) decides on a
publication, `require_audit_reader` (admin) reads the trail, and `require_pipeline_staff`
(admin, analyst, field_officer — the pre-R12 staff set) covers the read side of the dataset
tooling, which deliberately excludes the regulator. `test_p6_roles.py` pins all four.
...
| `GET /api/v1/data-gaps/matrix` | ● | ● | ● | ● | · |
| `GET /api/v1/data-gaps/network-plan` | ● | ● | ● | ● | · |
