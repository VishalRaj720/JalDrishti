...
The Singhbhum Shear Zone, in East Singhbhum and Saraikela-Kharsawan districts, hosts India's principal uranium deposits — Jaduguda, Bhatin, Narwapahar, Turamdih, Banduhurang, Mohuldih and Bagjata [43] — all of them conventional underground or open-pit operations run by the Uranium Corporation of India Ltd. [24]. The ore is uraninite in structurally controlled metamorphic host rock [43], [64], accompanied by sulphides — pyrite and chalcopyrite among them — in what is a uranium–copper–iron deposit [65]. None of these mines is an ISR operation, and this distinction is the premise of everything that follows.

In-situ recovery (ISR, also "in-situ leach") supplied 56 % of the uranium mined worldwide in 2022 [68]. Instead of excavating ore, an operator drills injection and recovery wells into a permeable, confined, sandstone-hosted ore body, injects a *lixiviant* — in the alkaline variant, native groundwater fortified with an oxidant and carbonate/bicarbonate — that dissolves uranium as uranyl-carbonate complexes, and pumps the pregnant solution back to the surface. A deliberate net over-extraction (the *bleed*, typically 0.5–3 % of injection) pulls groundwater inward and hydraulically contains the wellfield. Lixiviant that escapes past the ring of monitor wells is an *excursion*; after mining the aquifer must be *restored* by groundwater sweep, reverse osmosis and sometimes chemical reductants, and then monitored for stability [12], [13]. ISR mobilises not only uranium but sulphate, total dissolved solids, radium-226 and, depending on the ore, arsenic, selenium and molybdenum [15], [17], [63].
...
1. **No ISR-specific screening tool exists for Indian hard-rock hydrogeology.** The ISR screening literature concerns unconsolidated or weakly consolidated sandstone aquifers [12], [15]–[17], because the method needs an ore body in porous, water-saturated sand or sandstone, sealed by low-permeability beds on both sides so that the lixiviant can be steered [68]. Transferring that experience to a fractured metamorphic shear zone requires a stated model of fractured-rock transport — dual porosity, matrix diffusion, fracture-fabric anisotropy — and a statement of which of its parameters are measured locally (§4.4–§4.5, §5.1).
...
The project deliberately did not build a DRASTIC-type index (`docs/PRE_REPORT_AUDIT_AND_PLAN.md` §3 D-list). The reasons were data and question: three of DRASTIC's seven layers (soil, recharge, vadose zone) [53] would have been invented for Jharkhand rather than measured; and the question is not "is this aquifer susceptible to surface pollution" but "would an injected lixiviant at ore depth reach the shallow drinking-water aquifer" — a source-specific, vertical question that the 2.5-D screening of §4.5 addresses with the layers that *do* exist (the NAQUIM column, the measured water table and its seasonal swing). *The intrinsic-vulnerability literature does not provide a way to ask about a deep injected source, and it needs layers that do not exist at usable resolution for the state.*
...
The flow field a plume rides on is, in textbook practice, estimated from hydraulic heads at three or more wells by a plane fit [1]; the project generalises that to a distance-weighted plane fit on a 5 km grid from 398 CGWB stations, with head taken as elevation minus depth-to-water and a smoothed-DEM fallback where stations are sparse — the assumption, standard for hard-rock terrain, that the water table is a subdued replica of the topography [1]. Fracture fabric is mapped by the Geological Survey of India's lineament programme [42]; axial (doubled-angle) circular statistics are the standard way to average undirected orientations, and the circular variance measures alignment [71]. The vertical structure comes from CGWB's NAQUIM aquifer-mapping reports [37], [38], which for each district give the weathered-zone base, the depth range of productive fractures and the confined or semi-confined state of the deeper aquifer. Aquifer properties at the polygon scale are CGWB's; where a polygon carries no value the project fills from Freeze & Cherry's lithology-typical ranges [1], flagged as literature. The global permeability–porosity map GLHYMPS [62] was catalogued in the dataset survey but not used, because the CGWB polygons are finer and local.

*What the state-scale hydrogeological literature does not provide is any property at ore depth: every CGWB value characterises the drinking-water aquifer in the upper tens of metres, and the project's literature search found no published deep piezometry, packer test or tracer test for the Singhbhum Shear Zone.*

## 2.6 Monitoring systems and network design

CGWB's network-design practice is coverage-based — stations per unit area and distance to the nearest observation — and the first month's reading of monitoring-network design guidance [67] (MPR Nov 2025) confirmed that the classical objectives are spatial coverage, temporal frequency and representativeness. ISR-specific monitoring is prescriptive: NUREG-1569 [12] fixes the ring distance, the indicator panel, the sampling interval and the statistical form of the control limit (a simple percentage over baseline is permitted; the preferred rules — mean plus five standard deviations, or ASTM D6312 [61] — require a per-well temporal baseline). Optimal-network design in the research literature sets siting as an optimisation of an information objective, typically variance reduction of a kriged field or of a model's prediction.
...
A cyber-physical system (CPS) joins computation to a physical process: networked embedded computers observe and steer the process, typically in a feedback loop [70]. For environmental monitoring this report reads that as five layers — physical (sensors, telemetry), data (ingest, storage, provenance), computation (models), decision (thresholds, classifications) and action (alerts, control) — with feedback between them. The proposal's CPS genesis (proposal §10) described "environmental sensors, real-time data streams and machine learning algorithms in a unified monitoring framework … a closed-loop system." This report distinguishes a system with a sensing layer from a decision-support system that consumes a manual record; what makes a loop "closed" here is that a threshold crossing produces a delivered notification and an acknowledged action, whether or not the sensing is automatic.
...
3. **Fe, As, Mn never measured.** Iron and arsenic are 0 % populated in the 2023 file; manganese has no column; the 2000–2021 file has none of the three. The proposal named all three. The absence matters in the uranium belt specifically: the one published groundwater heavy-metal survey near two Singhbhum deposits (Bagjata and Banduhurang, ten samples each) found iron up to 5.3 mg/L and manganese up to 1.3 mg/L, above the drinking-water limits, while lead, copper and nickel were within them [23]. They are reported as `not_tested` on every well rather than passed over, and the alert scanner already reads them so that the first laboratory result to arrive raises an alert without anyone having to remember to add it.
...
7. **A demo field observation had been left in the tracked ore dataset — found while writing this report, and removed.** During the R11 test of the field-observation workflow (19 August 2026) a submission named `jharia` ("lots of Uranium") was approved and synced into the tracked ore files as an *added* record with a 400 m radius at 23.39° N 86.28° E, near Dhanbad, where no uranium deposit is known. The R11 fix `0533caa` stopped the belt envelope spreading from it, but the record itself remained, and at commit `476a4a9` the engine resolved a *deposit-tier* uranium source term (25,300 ppb) at that point. The sync had written it to two files: the deposit outlines (`Datasets/Jharkhand Ore/jharkhand_uranium_deposits.csv`) and the UDEPO grade sheet (`Datasets/udepo_uranium_deposits.xlsx`, row `FIELD-7fe0ffaf`). On 24 September 2026 the outline row was removed through the dataset manager's audited row-delete path, and the grade sheet — whose demo row carried no provenance marker, so the row-delete path refuses it as an original row — was restored to the IAEA export as first committed on 7 July 2026; the pin now resolves to no ore zone. The production database was checked and held no copy of the observation; its only registered site (Jaduguda) lies 82 km away, and no result in §6 was computed at or near the demo point. Two engine tests that had run only because the demo row existed now supply their own synthetic sighting. The lesson is recorded because it generalises: a test fixture that reads live data can pass for the wrong reason.
...
| Uranium attenuation k | log-triangular (0.05, 0.20, 0.70) /yr; mode by ore zone 0.35 / 0.28 / 0.12 | 0–0.70 | [18]; mineralogy tilt [64], [65] |
...
Each head minimises the pinball (quantile) loss of quantile regression [72], `L_q(u) = q·u` for u ≥ 0 and `(q − 1)·u` for u < 0, whose minimiser is the q-th conditional quantile. Hyper-parameters are fixed and recorded, not searched: 450 trees, depth 5, learning rate 0.05, subsample 0.85, column subsample 0.85, L2 regularisation 1.5, histogram method, seed 42 (`metrics.json["config"]`). The absence of a hyper-parameter search is stated in §5.3.
...
Raw quantile heads under-cover. The project applies split-conformal CQR [27]: scenarios are split into training and calibration folds (grouped, §4.6.5); on the calibration fold the conformity score `s_i = max(q̂₁₀(x_i) − y_i, y_i − q̂₉₀(x_i))` is computed; the ⌈(n + 1)(1 − α)⌉-th smallest score Q̂ widens both edges, `[q̂₁₀ − Q̂, q̂₉₀ + Q̂]`, with α = 0.20. The guarantee under exchangeability is that the widened band covers at least 80 % of new points from the same distribution regardless of how wrong the heads are. It is **Mondrian** [28]: the correction is computed per regime × species cell (eight cells), so fractured uranium does not borrow sulphate's error. And because rows of one scenario are correlated, the gate is **scenario-level** — at least 80 % of scenarios must have all their rows covered — and a finite-sample inflation `DELTA_INFLATE = 1.35` widens the calibration quantile to hold that stricter bar. The per-cell deltas (in log units) are in the model card and Appendix B.
...
| 40 | `jharia` demo observation: resolved a 25,300 ppb deposit-tier source at 23.39° N 86.28° E at `476a4a9`; removed 24 Sep 2026 (outline row via the dataset manager, grade row by restoring the 7 Jul 2026 UDEPO export); pin now resolves to no ore zone; production database held no copy | 3.5 | `resolve_inputs` at that point, 21 Sep 2026; `dataset.row_delete` and `dataset.restore` audit entries (local database); commits `0533caa`, `f4c79fd` |
...
| 47 | Faculty mentor: Asst. Prof. Mukesh Chandra | title page; declaration | all ten MPRs §2; confirmed by the student, 24 Sep 2026 |
| 48 | Citation corrections in `config/parameters.py` comments, made 23 Sep 2026: "Johnson et al. 2019" → Reimus et al. 2019 [18]; "BARC, J. Environ. Radioactivity 99 (2008) 1245" → Tripathi et al. 2008 [24] for the 23 mBq/L regional ²²⁶Ra value, and Tripathi et al. 2012 [66] for the <3.5–208 mBq/L potable-well range, which this report does not use. No computed value changed. | references; §3.6 (Table 3.3) | Crossref records for 10.1021/acs.est.9b01572 and 10.1016/j.apradiso.2007.12.019; publisher abstract of 10.1093/rpd/ncr014 |
