...
| `ModuleNotFoundError: No module named 'matplotlib'` (or `'xgboost'`) during a RUN, while /health and the maps are fine | The API imports the engine in-process, so the engine's runtime deps must be in `backend/requirements.txt` -- the image never installs `ml_pipeline/requirements.txt`. Both imports are reached only when a simulation actually executes, so nothing earlier catches it. Fixed 2026-08-25; if you add an engine dependency, add it there too |
| Simulation dies with no traceback, or the instance restarts mid-run | Out of memory. The engine loads lazily, so a green `/health` proves nothing about it; numpy, scipy, pandas, geopandas, xgboost and matplotlib together do not fit comfortably in 512 MB. Move to a 2 GB instance |
