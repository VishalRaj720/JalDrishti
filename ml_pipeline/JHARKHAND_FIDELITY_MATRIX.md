...
- **K, φ, T, thickness**: from the CGWB `Aquifers_Jharkhand.geojson` polygons —
  ...
  shear-zone correction once used T = 207–570 m²/day, real data but
  **misplaced** (2026-09-25, LIMITATIONS §1j): CGWB's own table labels those
  wells Tertiary sediments, 51–58 km east of the deposits. **Since 2026-09-26 it
  is Kudada's 19 m²/day**, the measured maximum near the mines, bounded from
  above by UCIL's own mine discharge (§1k).
...
*porous, confined sandstone* (IAEA NF-T-1.4 p.10: ~1 m/day advantageous,
<~0.1 m/day usually unfeasible). The old shear-zone K = 2.47 m/day sat inside
that window. The measured one does not reach its working level: ~0.23 m/day at
45 m and 0.03–0.18 m/day at the deposits' ore depths — below the IAEA's 0.1
m/day floor at five of the seven, and nowhere near 1 m/day at any. That is why
the ISR-feasibility level is served only as a labelled hypothetical (§1k).
...
|---|---|---|
| 1.1 | **Aquifer properties** (K, φ, T, b per polygon) | CGWB-published Jharkhand data. **D5 re-grounded 2026-09-26 (LIMITATIONS §1k):** the shear-zone transmissivity is Kudada EW's pumping test, **19 m²/day** (metasediments, 3 km from Turamdih), the measured maximum of the belt's placed host-rock tests. It is converted so the engine's K(z) law returns it over the tested interval. UCIL's mine discharge bounds the bulk T at ≤ 9 m²/day, so the baseline is on the conservative side. The retired 207–570 were Tertiary-sediment wells 51–58 km east (§1j). The ISR-feasibility level (IAEA, K ≈ 1 m/day) is served only as a labelled hypothetical beside it. |
...
| 1.5 | **Water-quality baselines** | Real CGWB wells; incremental-exceedance logic prevents blaming mining for natural background. **2026-09-26 (LIMITATIONS §1k):** uranium and Ra-226 around the mines come from four BARC mining-area surveys plus a 108-well regional survey (`Datasets/singhbhum_groundwater_radionuclide_baselines.csv`), blended continuously. Before this, the CGWB well nearest every deposit had no uranium value, so all deposit pins served an unsourced 1.0 µg/L; and radium was one statewide 23 mBq/L. |
...
| 3.11 | ✅ **FIXED (2026-09-25) — the vertical pathway moved water, not solute.** The upward-leakage time was `separation / (Kv·i/φ)` for every species, on Kv/Kh = 0.03 (chosen) and on K at ore depth for the whole column | At Jaduguda (20-yr run) uranium and TDS both "reached" the shallow aquifer in **17.5 yr**, and β 3 → 0 did not move it — while the same run's horizontal front retarded uranium ~270× through that β. The product paired its most optimistic horizontal answer with its most pessimistic vertical one, for the same rock | **Done (`P.VERTICAL_PATH`, `dashboard/vertical_path.py`, no retrain — the vertical screen is analytic-only):** (1) **matrix retention** on the path by the SAME Goltz–Roberts clock and β_eff = β·R_m as the horizontal front, β from the confining layer's own porosities; (2) **series K** — harmonic mean of the NAQUIM K(z) law over the column (Freeze & Cherry 1979) instead of K at ore depth (Jaduguda 0.563 → 1.32 m/day); (3) **Kv/Kh measured** — 0.12, the geometric mean of the five Maheshwaram-granite pumping-test values (Maréchal et al. 2004, J. Geol. Soc. India 63(5)), measured range 0.034–0.61 reported on every run; (4) **lixiviant indicators** (chloride, TDS, sulfate) screened on the same geometry, with `first_arrival` so a uranium run cannot hide the salts. Jaduguda 20 yr now: water 1.9 yr, **TDS 4.5 yr**, sulfate 22.7 yr, **uranium ~448 yr**, radium ~6,000 yr. The backend alerts key on `first_arrival`. **Still open:** Kv/Kh is a granite fissured-layer value <35 m deep, not SSZ schist at ore depth (the literature says schist is *less* anisotropic, so it may understate vertical flow); the upward gradient (0.005) remains a scenario assumption — a bleed-operated wellfield is a net sink, so the pathway is realistic only locally around injection wells or in a natural discharge setting; foliation dip (~35–40° in the SSZ, UCIL) is **not** modelled — no hydraulic measurement of along-foliation vs cross-foliation K exists to rotate a tensor with |
