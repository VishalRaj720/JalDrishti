...
| 3.7 | ⏸️ **DEFERRED BY CHOICE — NOT data-blocked.** No monsoon transients (recharge pulses, seasonal flow reversal near divides, water-table swing in the vertical module) | Jharkhand's defining hydrologic feature is represented only as band width | **Data is NOT the blocker** — seasonal pre/post-monsoon water levels are already on disk (`cgwb_waterlevel_jharkhand.csv`) and `flow_field.py` already derives `seasonal_amp_effective` from them. This is a modelling task (two-season alternating gradient in `front_position`; a full transient needs a numeric solver), deliberately deferred 2026-08-02 at the user's request. **It is the only open item that is not data-limited.** |
